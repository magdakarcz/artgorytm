...
void main()
{
    ...
    float reZ = pattern.minX + t.x * (pattern.maxX - pattern.minX);
    float imZ = pattern.minY + t.y * (pattern.maxY - pattern.minY);
    ...
    if( abs(convergenceSpeed - 1.0 ) < 0.0001 )
        ...
        fragColor = texture( colormap, vec2( convergenceSpeed, 0.0 ) );
